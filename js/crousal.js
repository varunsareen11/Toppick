class CgCarousel {
  ...
  setSwipeDirection(e) {
    ...
    if (
      Math.abs(distX) >= this.swipeThreshold &&
      Math.abs(distY) <= this.swipeRestraint
    ) {
      this.swipeDir = distX < 0 ? "left" : "right";
    } else if (
      Math.abs(distY) >= this.swipeThreshold &&
      Math.abs(distX) <= this.swipeRestraint
    ) {
  ...
  onSwipeMove(e) {
    ...
    if (["left", "right"].includes(this.swipeDir) && e.cancelable)
      e.preventDefault();
  }
  ...
  checkBreakpoint() {
    ...
    const options = currentBreakpoint
  ...
  setButtonsVisibility() {
    ...
    this.isButtonRightDisabled =
      !this.options.loop && this.currentIndex === this.maxIndex - 1;
  }
  ...
  animateLeft(timestamp, trans, gap, duration) {
    ...
    const dist = (
      trans * progress +
      this.animationCurrentTrans * (1 - progress)
    ).toFixed(2);
  ...
  clearInfinite() {
    this.isInfinite = false;
    this.track.style.left = `calc(${this.currentIndex * -100}% - ${
      this.options.spacing * this.currentIndex
    }px)`;
  ...
  next() {
    const nextIndex =
      this.currentIndex === this.maxIndex - 1 ? 0 : this.currentIndex + 1;
  ...
  prev() {
    const nextIndex =
      this.currentIndex === 0 ? this.maxIndex - 1 : this.currentIndex - 1;
    if (!this.options.loop && nextIndex > this.currentIndex) return;
